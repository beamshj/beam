          ...
          {BannerSliderData.slides.map((slide, index) => (
            <SwiperSlide key={index}> 
              <div key={index} className="slide h-full w-screen relative overflow-hidden text-white">
                ...
                <div key={`${index}-${textVersion}`} className="h-full w-full -z-40 absolute bg-[linear-gradient(180deg,_rgba(0,0,0,0)_21.7%,_rgba(0,0,0,0.6)_63.57%,_rgba(0,0,0,0.8)_100%)]"></div>
                  ...
                  <div className="absolute w-full h-full">
                    ...
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.6, ease: 'easeOut' }} className="text-3xl  text-white leading-[1.1] font-custom font-light lettersp-4" >
                              {slide.titleblue} {slide.titlewhite}</motion.h1>
                          ...
                          <div className="">
                            <div className='mt-5 w-fit md:mt-10 p-[1px] group transition-all duration-300 bg-bdrcolor bg-[linear-gradient(90deg,_#42BADC_0%,_#12586C_100%)] rounded-full hover:-translate-x-2 hover:shadow-[0_0_15px_rgba(66,186,220,0.5)] '>
                              <a href={'#'} className='cursor-pointer px-2 md:px-4 py-2 md:py-3 bg-primary rounded-full flex items-center gap-2 transition-all duration-300'>
        ...
        <div className="absolute bottom-[30%] md:bottom-[37%] w-full ">
          <div className="container flex justify-end">
            <span className="text-[15px] text-white whitespace-nowrap font-light relative -right-3 md:right-2 z-10 flex flex-col items-center">
              ...
              <span className="mt-4 -rotate-90 font-light text-[15px]">{`0${totalSlides}`}</span> <span className="font-medium w-[1px] h-[8px] bg-white mt-1"></span> 
