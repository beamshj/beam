...
export async function GET(request: NextRequest) {
  try {
    ...
    if (id) {
      const foundNews = news.categories
        .flatMap((category: { news: { _id: string } }) => category.news)
        .find((news: { _id: string }) => news._id.toString() === id);
      ...
      return NextResponse.json(
        { data: foundNews, message: "News fetched successfully" },
        { status: 200 }
      );
    } else if (slug) {
      const foundNews = news.categories
        .flatMap((category: { news: { slug: string } }) => category.news)
        .find((news: { slug: string }) => news.slug === slug);
      console.log(foundNews);
      if (!foundNews) {
        return NextResponse.json(
          { message: "News not found" },
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (id) {
      ...
      const toUpdate = news.categories
        .flatMap((category: { news: { _id: string } }) => category.news)
        .find((news: { _id: string }) => news._id.toString() === id);
      if (!toUpdate) {
        return NextResponse.json(
    ...
    const news = await News.findOneAndUpdate({}, body, {
      upsert: true,
      new: true,
    });
    if (!news) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const category = news.categories.find(
      (category: { name: string }) => category.name === body.category
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    newsDoc.categories = newsDoc.categories.map(
      (category: { news: { _id: string }[] }) => {
        const hasNews = category.news.some(
          (n: { _id: string }) => n._id.toString() === newsId
        );
        if (hasNews) {
          newsDeleted = true;
          return {
