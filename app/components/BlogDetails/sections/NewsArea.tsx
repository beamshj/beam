...
const NewsArea = ({ data }: { data: BlogType["categories"][number]["blogs"][number] }) => {
    ...
    useEffect(() => {
        const boldElements = document.querySelectorAll(".blog-content p strong span, .blog-content span p");
    ...
    useEffect(() => {
        const elements = document.querySelectorAll<HTMLElement>('[style*="background-color"][style*="color"]');
                    ...
                    <div className="relative">
                        ...
                        >
                            <li className="text-black text-sm font-light list-disc">
                                {readingTime} {hasArabic ? "قراءة دقيقة" : "mins read"}
                            </li>

                            {typeof window !== "undefined" && window.innerWidth > 824 && (
                                <div className="">
                                    <AnimatePresence>
                                        {showIcons && (
                                            <motion.div
