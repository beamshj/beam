...
const animateContentIn = () => {
  ...
  gsap.fromTo(
    targets,
    { opacity: 0, y: 30, filter: "blur(4px)" },
    { opacity: 1, y: 0, filter: "blur(0px)", duration: 0.9, ease: "power3.out", stagger: 0.3 }
  );
};
...
const animateEntry = (container: HTMLElement) => {
  ...
  const tl = gsap.timeline({ onComplete: () => { root.remove(); animateContentIn(); } });
...
const animateCinematicCurtain = (
  ...
  const swiperWrapper = container.querySelector(".swiper") as HTMLElement | null;
  if (swiperWrapper && swiperWrapper.style.opacity !== "0") swiperWrapper.style.opacity = "0";
  ...
  const applyGradient = (c: CanvasRenderingContext2D, w: number, h: number) => {
    const g = c.createLinearGradient(0, 0, 0, h);
    GRADIENT_STOPS.forEach(({ stop, alpha }) => g.addColorStop(stop, `rgba(0,0,0,${alpha})`));
    c.globalAlpha = 1;
    c.fillStyle   = g;
    c.fillRect(0, 0, w, h);
  };

  const coverDraw = (c: CanvasRenderingContext2D, img: HTMLImageElement, w: number, h: number, dx = 0, alpha = 1) => {
    if (!img.naturalWidth) return;
    const s  = Math.max(w / img.naturalWidth, h / img.naturalHeight);
    const iw = img.naturalWidth  * s;
    const ih = img.naturalHeight * s;
  ...
  const cleanup = () => {
    ...
    gsap.to(canvas, {
      ...
      onComplete: () => { done = true; canvas.remove(); onDone(); },
    });
  };

  function runTimeline() {
    const tl = gsap.timeline({ onComplete: cleanup });
    tl.to(state, { radius: MAX_R * 1.2, duration: 1.1, ease: "power2.inOut" }, 0);
    tl.to(state, { prevDrift: direction * W * 0.03, prevOpacity: 0, duration: 0.75, ease: "power1.inOut" }, 0.15);
  }

  let loaded = 0;
  const onLoad = () => { loaded++; if (loaded === 2) runTimeline(); };
  if (imgPrev.complete) onLoad(); else imgPrev.onload = onLoad;
  if (imgNext.complete) onLoad(); else imgNext.onload = onLoad;
  setTimeout(() => { if (!done) cleanup(); }, 3000);
};
...
const HeroSection = ({ data }: { data: HomeProps["bannerSection"] }) => {
  ...
  useEffect(() => { preloadImages(t.items.map((item) => item.image)); }, [t.items]);
  ...
  const handleBeforeTransition = useCallback((swiper: SwiperClass) => {
    if (isAnimating.current) return;
    if (!sectionRef.current) return;
    ...
    const total = t.items.length;
    let delta = nextIndex - prevIndexRef.current;
    if (Math.abs(delta) > total / 2) delta = delta > 0 ? delta - total : delta + total;
    directionRef.current = delta >= 0 ? 1 : -1;
  ...
  useEffect(() => {
    ...
    const onLeave = () => { pointerDown.current = false; };
