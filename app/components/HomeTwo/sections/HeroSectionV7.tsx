...
// ─── Entry Animation — "BEAM Logo Rays" ──────────────────────────────────────
//
//  Pixel-faithful recreation of the BEAM logo:
//  — Solid cyan (#42BADC) fills the entire frame as background
//  — 18 crisp SOLID WHITE rays radiate from the exact bottom-left corner,
//    fanning upward and rightward across the frame — matching the logo exactly
//  — Rays are thick opaque white strokes with real pixel width, no glow/blur
//  — They deploy sequentially left→right across the fan with fast stagger
//  — Rays are shortened so they don't reach the opposite edge
...
const animateEntry = (container: HTMLElement) => {
  ...
  // ── Origin — exact bottom-left corner ─────────────────────────────────────
  const OX = 0;
  const OY = H;
  ...
  const rays: Ray[] = Array.from({ length: RAY_COUNT }, (_, i) => {
    ...
    const thickness = Math.max(2, 20 - t * 8);
    return { angleDeg, thickness, progress: 0 };
  });
  ...
  const draw = () => {
    ...
    canvas.style.opacity = String(state.canvasOpacity);
