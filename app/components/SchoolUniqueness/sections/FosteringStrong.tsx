    ...
    <section className="py-8 md:py-12 lg:py-20 2xl:py-[135px]">
      <div className="container">
        <div className="grid lg:grid-cols-[57%_40%] gap-[3%] ">
          ...
          <div>
            ...
            <div className="mt-4 md:mt-6 xl:mt-8 2xl:mt-12">
              ...
              {fsData.items.map((item, index) => {
                return (
                  <motion.div variants={moveUp(index * 0.2)} initial="hidden" whileInView="show" viewport={{amount: 0.1, once: true}} className="flex gap-3 border-b border-[#D3D3D3]" key={index}>
                    <motion.div variants={moveUp(0.5)} initial="hidden" whileInView="show" viewport={{amount: 0.1, once: true}} key={index} className="flex  items-center gap-3 pt-7 pb-5 group cursor-pointer">
          ...
          {/* Right Image */}
          <motion.div variants={moveUp(1.25)} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.1 }} className="relative w-full h-[250px] lg:h-auto rounded-[12px] overflow-hidden lg:ms-10 hidden xl:block" >
