          ...
          >
            {currentData.items.map((item, index) => (
              ...
              >
                ...
                <div className={`${isActive === index ? "h-full" : " h-0   "  } absolute bottom-0 left-0 w-full h-0 group-hover:h-full transition-all duration-300 bg-gradient-to-b from-[#42BADC] to-[#7E5AA3]/76 z-10`}></div>
                <div className="flex flex-col h-full justify-between relative z-30">
                  <div className="mb-4 flex flex-col relative overflow-hidden">
                    <div className={`${isActive === index ? "opacity-100 -translate-y-full" : "   "  }  group-hover:opacity-100 group-hover:-translate-y-full   transition-all duration-300`}>
                    ...
                    <div className={`${isActive === index ? "opacity-100 -translate-y-full" : " opacity-0 "  } group-hover:opacity-100 group-hover:-translate-y-full      mt-6 transition-all duration-300`}>
                      <h3 className="text-lg xl:text-xl font-light text-white leading-[1.2]">{item.title}</h3>
                    </div>
                  </div>
                  <div className="flex flex-col justify-end relative overflow-hidden min-h-[4rem]">
                    <div className={`${isActive === index ? "opacity-0 -translate-y-full" : " "  } group-hover:opacity-0 group-hover:-translate-y-full   transition-all duration-300`}>
                      <h3 className="text-lg font-light leading-[1.2] text-black mb-3">{item.title}</h3>
                    </div>
                    <div className={`${isActive === index ? "opacity-100 translate-y-0" : "opacity-0 translate-y-full"  } group-hover:opacity-100 group-hover:translate-y-0 absolute bottom-0 left-0 w-full  transition-all duration-300`}>
                      <p className="text-sm font-light text-white leading-relaxed max-w-[30ch]">{item.desc}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
