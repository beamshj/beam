...
export default function Accreditation() {
  ...
  const handleEditCategory = async (id: string) => {
    try {
      const response = await fetch(
        `/api/admin/accreditations/category?id=${id}`,
        {
          method: "PATCH",
          body: JSON.stringify({ name: category, name_ar: categoryArabic }),
        },
      ...
      if (response.ok) {
        ...
        setAccreditationsList(
          data.data.categories.flatMap(
            (category: { accreditations: { _id: string }[] }) =>
  ...
  const getTaskPos = (id: number | string) =>
    accreditationsList.findIndex((item: { _id: string }) => item._id == id);
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setAccreditationsList(
      (accreditationsList: { _id: string; title: string }[]) => {
        ...
        <div className="h-screen w-full p-5 shadow-md border-gray-300 rounded-md overflow-y-hidden bg-white">
          <div className="flex justify-between border-b-2 pb-2">
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  reorderMode ? confirmPosition() : setReorderMode(!reorderMode)
                }
              >
              ...
              {!reorderMode && (
                <Button
                  onClick={() => router.push("/admin/accreditations/add")}
                >
                  Add Accreditation
                </Button>
              )}
          ...
          <div className="mt-2 flex flex-col gap-2 overflow-y-scroll h-[90%]">
            {reorderMode && (
              ...
              >
                <SortableContext
                  items={accreditationsList.map((item) => item._id)}
              ...
              accreditationsList?.map((item) => (
                ...
                >
                  ...
                  <div className="flex gap-5">
                    <MdEdit
                      onClick={() =>
                        router.push(`/admin/accreditations/edit/${item._id}`)
                      }
                    />
                    ...
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <div className="flex gap-2">
                          ...
                          <DialogClose
                            className="bg-black text-white px-2 py-1 rounded-md"
                            onClick={() => handleDeleteAccreditation(item._id)}
                          >
                            Yes
                          </DialogClose>
                        </div>
